...
import {
  ...
  getFromSupabaseGlobal,
  getFromSupabaseLocal,
  addToSupabaseGlobal,
  addToSupabaseLocal,
  updateToSupabaseGlobal,
  updateToSupabaseLocal,
} from "./(crud-supabase)/definitions";
  ...
  try {
    ...
    if (requestedChars > MAX_GLOBAL_CHARS_PER_DAY) {
      new BackendLogger(
        ...
        await getRemainingLocalChars(clientIP),
      );
      return new Response(JSON.stringify({ error: violations[1] }), {
        status: 400,
      });
      ...
      return new Response(JSON.stringify({ error: violations[2] }), {
        status: 400,
      });
    } else {
      if (localResponse && globalResponse) {
        ...
        if (total_global_chars + requestedChars > MAX_GLOBAL_CHARS_PER_DAY) {
          ...
          return new Response(JSON.stringify({ error: violations[3] }), {
            status: 400,
          });
        } else if (
          ...
          return new Response(JSON.stringify({ error: violations[4] }), {
            status: 400,
          });
        } else {
          new BackendLogger(
...
export const deepLTranslationApi = async (
  transcript,
  sourceLanguage,
  targetLanguages,
) => {
  try {
    ...
    for (const targetLanguage of targetLanguages) {
      const result = await translator.translateText(transcript, "en", "de");
